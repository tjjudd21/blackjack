//Trevor Judd and Gabriel Marinescu
...
public class Decks
{
   ...
   public Decks(int num)
   {
      ...
      for (int i = 0; i < numDecks; i++)
      {
		...
		for (int index = 1; index <= STANDARD_DECK_SIZE; index++)
		{
			decks[count] = new Card(index);
			count++;
		}
      }
   ...
   public void shuffle()
   {
	  ...
	  for (int i = 0; i < numDecks; i++)
	  {
        ...
        Card[] newDeck = new Card[STANDARD_DECK_SIZE];
        Permutation shuffledDeck = new Permutation(STANDARD_DECK_SIZE, STANDARD_DECK_SIZE);

        for (int j = 0; j < STANDARD_DECK_SIZE; j++)
		{
            newDeck[shuffledDeck.next() - 1] = decks[i * STANDARD_DECK_SIZE + j];
		}

        for (int k = 0; k < STANDARD_DECK_SIZE; k++)
		{
            decks[i * STANDARD_DECK_SIZE + k] = newDeck[k];
		}
